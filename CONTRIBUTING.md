...
This repo packages [ArchiveBox](https://github.com/ArchiveBox/ArchiveBox) for StartOS.
...
ArchiveBox runs the `archivebox/archivebox` image pinned via `dockerTag` in `startos/manifest/index.ts`.

1. Bump `dockerTag` in `startos/manifest/index.ts` to `archivebox/archivebox:<new version>`.
